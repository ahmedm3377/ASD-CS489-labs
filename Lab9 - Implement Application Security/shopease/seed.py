...
def seed_all(database_url: str | None = None):
    ...
    # customers
    # create users with hashed passwords and roles
    c1 = Customer(firstName="Alice", lastName="Smith", email="alice@example.com", phone="1234567890", address="1 Main St", password=get_password_hash("pass"), role="customer")
    c2 = Customer(firstName="Bob", lastName="Jones", email="bob@example.com", phone="0987654321", address="2 High St", password=get_password_hash("pass"), role="customer")
    # an agent user and a manager user stored in customers table for auth simplicity
    a_user = Customer(firstName="Tom", lastName="Agent", email="tom.agent@example.com", password=get_password_hash("agentpass"), role="agent")
    m_user = Customer(firstName="Manny", lastName="Manager", email="manny.manager@example.com", password=get_password_hash("managerpass"), role="manager")
    session.add_all([c1, c2, a_user, m_user])
    session.commit()
